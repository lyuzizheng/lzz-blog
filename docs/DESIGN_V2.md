...
>
> **BRAWUKA-83 迭代（2026-09-06 创始人反馈）**：取消"叠放 → 点击散落"状态机——四张章节胶片**常驻散落**、各自直接是链接（保留 hover 回正上浮 + 显影、idle 呼吸、数字键 1–4）；工作台下半区新增**模糊底片堆层**（6~8 张无标签负片，低透明度 + `blur(1.5px)`，纯装饰 pointer-events:none）；背景层追加**工作台 SVG 蚀刻**（灯箱台面内发光、刻度尺、套准标记、胶片罐、放大镜头——钴蓝 1px 单线族）。
