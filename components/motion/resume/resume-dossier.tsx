 ...
 * - 展开动效使用 CSS grid-template-rows 过渡（无 JS layout 动画）。
 * - tbd 条目标记 [NEEDS-OWNER] 且默认收进"待确认"提示，不伪装成已验证事实。
 * - prefers-reduced-motion 经全局 CSS 门禁降级为瞬时切换。
 */
      ...
      {CAPABILITY_DIMENSIONS.map((dim, index) => {
        ...
        return (
          ...
          >
            ...
            {/* Accordion drawer: CSS grid-rows transition — no JS layout
                animation, no framer-motion height tween. */}
            <div
