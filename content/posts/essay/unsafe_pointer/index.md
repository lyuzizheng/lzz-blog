...
I work as the backend developer of the Instant Messaging (IM) Team in our company. The banner in the first picture, the action bar in the second picture are under my domain where they are dynamically loaded from the server. Therefore translation problems server were exceedingly rare.  

Normally, all app client requests will attach common parameters including app display language (e.g., `lang=EN`) by the App network framework. Its impossible that the issue came from the client side. I started to check the log and realised a lot of error logs during the weekend. It started on Friday afternoon, and yes, that timing was crucial.  

![issue](errorlogs.jpeg#center)

Two things were wired, firstly these erros logs were outside of our usual tracing framework, suggesting they were produced elsewhere, possibly by an external sidecar or SDK error. Secondly, the **rate of error** occurrences was increasing, not total number of logs, indicating that the problem was deteriarating.

Just as I delved into the logs to identify the source of the issue, a colleague from the language translation SDK team reached out to me via Lark, our internal messaging platform. He informed me that our service had been bombarding the translation server with requests for **non-existing languages**, and this was causing undue stress on the server.  

I need to pause here and explain to you how translation worked on TikTok. The banner displayed on the chat page (Fig. 1) was a pre-translated, fixed notice available in 150+ languages. The server retrieved these `fixed strings` from the language SDK using a KEY+Language combination.

![sdk](sdk1.png#center)  

So these error logs are from the SDK but why these error logs have little context info? My colleague then explained the inner workings of the SDK: it would cache query results with the KEY+LANG combination, updating the cache with a batch update every 30 seconds to fetch any new fixes. So the logs are actually deteched with regular framework.  
...
Right now I knew what the issue was but what was the cause?  Luckily, I also knew the cuase, because I did a server version release and upgraded that microservice on Friday Afternoon (forgive me). The release involved six stacked commits, one of which must be responsible for the language parameter issue.  

So after I rolled back the release, I did a thorough inspection on all code changes among these 6 commits. I was pretty sure all of them were clear and fine! Also, the automation test, on the online environment was error free!! FML, what happened! I then had to choose release the commit changes one by one and test it out on the online env.  
...
Yet, yet, this will not cause any issue as normal, the header string will not be accessed after the request ends. However, we have the SDK's continuous polling every 30 seconds of all the **unsafe cached lang string** within the mmomory, leading to a snowball effect of language glitches on our users' phones.

![hertz](whole.png#center)  

So, let this be a cautionary tale of how a seemingly innocent upgrade can unleash language pandemonium. The lesson here? Next time we delve into the realms of "black magic" code, let's have our wizard hats and potions ready for thorough testing and maybe even a sprinkle of fairy dust for good luck! 
