...
export default defineConfig({
      ...
      schema: s
        .object({
          ...
          content: s
            ...
            .pipe(
              s.mdx({
                // Emit real ESM modules (not eval-able function bodies): the
                // Cloudflare Workers runtime forbids eval/new Function, so any
                // worker-side render of a post page must import a real module.
  ...
  // After the build, emit each post's compiled MDX as a real .mjs module under
  // .velite/mdx/<slug>.mjs plus a static loader map. MdxContent lazy-imports
  // through this map — no eval/new Function anywhere in the render path.
  complete: async data => {
    const mdxDir = path.resolve('.velite/mdx')
    fs.rmSync(mdxDir, { recursive: true, force: true })
    const entries: string[] = []
    for (const post of data.posts) {
      const file = path.join(mdxDir, `${post.slug}.mjs`)
      fs.mkdirSync(path.dirname(file), { recursive: true })
      fs.writeFileSync(file, post.content)
      entries.push(`  ${JSON.stringify(post.slug)}: () => import(${JSON.stringify(`./${post.slug}.mjs`)}),`)
    }
    fs.writeFileSync(
      path.join(mdxDir, 'index.ts'),
      [
        '// Auto-generated by velite.config.ts (complete hook). Do not edit.',
        'export const mdxModules: Record<string, () => Promise<{ default: unknown }>> = {',
        ...entries,
        '}',
        ''
      ].join('\n')
